...
@router.post("", response_model=SAdresses, summary="Добавить адрес", status_code=status.HTTP_201_CREATED)
async def add_adress(data: SAdressesAdd):
    return await AdressesService.create(data)


@router.patch("/{id}", response_model=SAdresses, summary="Обновить адрес")
...
@router.delete("/{id}", response_model=Optional[SAdresses], summary="Мягкое удаление адреса")
